...
### Normative approach to skills mismatch by gender
...
n_skills <- LFS_clean %>% # summary table
  ...
  group_by(year, sequence, quarter, SEX, sk_mm, COUNTRY2) %>% 
  summarise(count = sum(PWT)) %>% 
  group_by(year, sequence, quarter, SEX, COUNTRY2) %>% 
...
p_nskills <- n_skills %>% 
  ...
  labs(x = "Year", 
       y = "Percentage", 
       title ="Figure 1: Overeducated workers by gender",
       subtitle  = "By gender",
       caption = ""
       ) +
...
### Normative approach to skills mismatch by age
...
n_skills_age <- LFS_clean %>% # summary table
  ...
  filter(ILODEFR == 1) %>% # only people in employment
  filter(INECAC05 %in% c(1, 2)) %>% #only employees and self-employed
  group_by(year, sequence, quarter, AGE_cat, sk_mm, COUNTRY2) %>% 
  summarise(count = sum(PWT)) %>% 
...
nskills_age_total <- n_skills_age %>% 
  ggplot() + 
...
p_nskills_age / nskills_age_total

```



### Normative approach to skills mismatch by educational level
...
nskills_total_lvl <- n_skills_lvl %>% 
  ggplot() + 
...
p_nskills_lvl / nskills_total_lvl
...
### Normative approach to skills mismatch by gender an by country

```{r normative approach to skills mismatch table, echo=FALSE}

p_nskills_grid <- n_skills %>% 
  ggplot() + 
  geom_line(aes(x = sequence, y = percentage, colour = factor(SEX)), size = 0.5, show.legend = FALSE) +  
  geom_point(aes(x = sequence, y = percentage, colour = factor(SEX)), shape = 15, show.legend = TRUE) +
  facet_wrap(~ COUNTRY2)+
  ...
  # scale and colour
  scale_color_discrete_qualitative(palette = "Dark 3") +
  scale_y_continuous(breaks = seq(0,0.2, 0.05), limits = c(0, 0.2), labels =
                       scales::percent_format(accuracy = 1L), expand = c(0,0), sec.axis = dup_axis()) +
  ...
  labs(x = "Year", 
       y = "Percentage", 
       title ="Figure 8: Overeducated workers by gender",
       subtitle  = "By country",
       caption = ""
       ) +
