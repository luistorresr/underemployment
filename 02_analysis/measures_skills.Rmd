...
```{r plot skills overeducated employees, fig.width=7, fig.height=3, fig.fullwidth=TRUE, fig.cap= "workers who have a qualification above the required for their occupation"}
...
nskills_total <- n_skills %>% 
  ...
  labs(x = "Year", 
       y = "Total", 
       title = "Overeducated workers (total)",
  ...
  theme_economist_white(gray_bg = FALSE, base_family = "Lato") +
  theme(plot.margin = unit(c(0, 0, 0, 0), "lines"),
        plot.title = element_text(size = 14, margin = ggplot2::margin(5, 0, 5, 15)),
        plot.subtitle = element_text(size = 12, hjust = 0, margin = ggplot2::margin(5, 0, 15, 0)),
        plot.caption = element_text(size = 10, hjust = 1, margin = ggplot2::margin(2, 0, 2, 0)),
        axis.title.x = element_text(margin = ggplot2::margin(t = 8), size = 12),
