...
N_total <- LFS_clean %>% 
  ...
  summarise(Population = sum(na.omit(PWT[AGE >= 18 & AGE <= 64])), # everyone between 18 to 64
            Active = sum(na.omit(PWT[ILODEFR <=2 & AGE >= 18 & AGE <= 64])), # in employment and unemployed between 18 to 64
            Workers = sum(na.omit(PWT[ILODEFR == 1 & INECAC05 <= 2 & AGE >= 18 & AGE <= 64]))) %>% # employee and self-employees between 18 to 64
...
## Indicator 1: Part-timers who could not find a full-time job

Our analysis explores trends by personal characteristics, job arrangements, and sector division (geographic and industrial)  
...
```{r plot involuntary part time by sex, fig.width=9, fig.height=7, fig.fullwidth=TRUE, fig.cap= "Figure 1: Male and female part-time workers who could not find a full-time job"}
...
p_ptime <- t_ptime %>% 
  ...
  labs(x = "", 
       y = "Percentage", 
       title ="Part-time male and female workers who could not find a full-time job",
       subtitle  = "",
...
```{r plot involuntary part time age, fig.width=9, fig.height=6, fig.fullwidth=TRUE, fig.cap= ""}
